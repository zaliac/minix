/* This process is the father (mother) of all MINIX user processes(用户进程).  When
 * MINIX comes up, this is process 2(进程2).  It executes the /etc/rc shell file and
 * then reads the /etc/ttys file to find out which terminals need a login
 * process(执行shell,查看ttys文件,找出哪一个终端需要登录进程.).
 */
