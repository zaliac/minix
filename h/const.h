/* 1 common constants for kernel. */

#define EXTERN        extern	/* used in *.h files */
#define PRIVATE       static	/* PRIVATE x limits the scope of x .static数据被存储于.data, .bss区，函数调用结束后，static变量的值仍然会得到保留。*/
#define PUBLIC			/* PUBLIC is the opposite of PRIVATE 全局变量,定义在函数外,链接后,其它文件可用.*/
...
#define NR_TASKS           8	/* 任务槽位number of tasks in the transfer vector */
#define NR_PROCS          16	/* 进程槽位number of slots in proc table */
#define NR_SEGS            3	/* # segments per process */
#define T                  0	/* proc[i].mem_map[T] is for text  代码区 */
#define D                  1	/* proc[i].mem_map[D] is for data  数据区 */
#define S                  2	/* proc[i].mem_map[S] is for stack 堆栈区 */

#define MAX_P_LONG  2147483647	/* 2G-1:long值最大正整数.maximum positive long, i.e. 2**31 - 1 */
