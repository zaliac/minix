/* 15 This file contains a driver for a Floppy Disk Controller (FDC) using the
...
/* Carry out a read or write request from the disk. */
  ...
  while (errors <= MAX_ERRORS) {
	...
	/* 1 Now set up the DMA chip. */
	dma_setup(fp);

	/* 2 See if motor is running; if not, turn it on and wait */
	start_motor(fp);

	/* 3 If we are going to a new cylinder, perform a seek. */
	r = seek(fp);
