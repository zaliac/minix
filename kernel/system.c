/* 18 (system task,系统任务,内核与内存/文件系统的数据交换接口.)
 ...
 *
 *   SYS_FORK	 informs kernel that a process has forked 通知内核,创建了一个子进程.
 *   SYS_NEWMAP	 allows MM to set up a process memory map 让内存创建一个进程的内存映像.
 *   SYS_EXEC	 sets program counter and stack pointer after EXEC 在EXEC执行后设置CS:PC和栈指针SP.
 *   SYS_XIT	 informs kernel that a process has exited 通知内核一个进程已退出.
 *   SYS_GETSP	 caller wants to read out some process' stack pointer 调用者想知道某进程的栈指针SP.
 *   SYS_TIMES	 caller wants to get accounting times for a process 调用者想知道某进程的执行次数.
 *   SYS_ABORT	 MM or FS cannot go on; abort MINIX 紧急状态,FS和MM无法运行,minix需要退出.
 *   SYS_SIG	 send a signal to a process 向某一进程发信号.
 *   SYS_COPY	 requests a block of data to be copied between processes 在进程间拷贝一块数据.
 *
...
/*===========================================================================*
 *				do_fork	设置子进程进程表信息,并关闭它,不让它在内存映象未设置好之前运行. *
 *===========================================================================*/
...
/* Handle sys_fork().  'k1' has forked.  The child is 'k2'. */
  ...
  int k1;			/* 父进程槽号number of parent process */
  int k2;			/* 子进程槽号number of child process */
  int pid;			/* 子进程号process id of child */
  int bytes;			/* counter for copying proc struct */

  k1 = m_ptr->PROC1;		/* 个extract parent slot number from msg */
  ...
  while (bytes--) *dptr++ = *sptr++;	/* 将父进程表数据拷贝到子进程槽中.copy parent struct to child */

  rpc->p_flags |= NO_MAP;	/* 禁止新建子进程运行.inhibit the process from running */
  rpc->p_pid = pid;		/* 设子进程的父进程ID号install child's pid */
  rpc->p_reg[RET_REG] = 0;	/* child sees pid = 0 to know it is child */

  rpc->user_time = 0;		/* 初使化子进程使用时间为0.set all the accounting times to 0 */
  rpc->sys_time = 0;
  rpc->child_utime = 0;
  rpc->child_stime = 0;
  return(OK);
 ...
 *				do_newmap 拷贝内存映象
 *				struct mem_map {
  	  	  	  	  	  	  	  	  vir_clicks mem_vir;		//虚地址 virtual address
  	  	  	  	  	  	  	  	  phys_clicks mem_phys;		//实地址 physical address
