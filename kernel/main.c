/* 13 第一个可执行代码.tools包内bootlbock.s,fschk.s运行后执行的内核起始程序. */
...
/* Start the ball rolling. */
  ...
  register int t;
  vir_clicks size;									//typedef unsigned vir_clicks;
  phys_clicks base_click, mm_base, previous_base;	//typedef unsigned phys_clicks;
  phys_bytes phys_b;								//typedef long phys_bytes;
   ...
   * sp, since the 3 words prior to it will be clobbered(使惨败) when the kernel pushes
  ...
  base_click = BASE >> CLICK_SHIFT;		// CLICK_SHIFT=4
  ...
  /* 设定中断向量表(将中断与中断处理函数关联).Set up the new interrupt vectors. */
