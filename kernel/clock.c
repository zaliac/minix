/* 16 This file contains the code and data for the clock task.  The clock task
...
#define MILLISEC         100	/* 0.1s,how often to call the scheduler (msec) */
...
PRIVATE int (*watch_dog[NR_TASKS+1])();	/* 报警指向的被调函数数组.watch_dog functions to call */
 ...
 */
  ...
  while (TRUE) {
     receive(ANY, &mc);		/* 接收消息.go get a message */
     opcode = mc.m_type;	/* extract the function code */

     switch (opcode) {
	case SET_ALARM:	 do_setalarm(&mc);	break;	//用户进程请求设定时器.
	case GET_TIME:	 do_get_time();		break;
	case SET_TIME:	 do_set_time(&mc);	break;
	case CLOCK_TICK: do_clocktick();	break;	//CPU处理时钟中断时,发送消息给时钟驱动程序.调用do_clocktick()函数.
 ...
 */
  ...
  register struct proc *rp;//将指向进程的指针存入一个寄存器中.
  int proc_nr;			/* 哪个进程申请报警.which process wants the alarm */
  long delta_ticks;		/* 这个进程需求设定多少报警等待时间.in how many clock ticks does he want it? */
  int (*function)();		/* 函数指针指定被调函数. function to call (tasks only) */

  /* Extract the parameters from the message. */
  proc_nr = m_ptr->CLOCK_PROC_NR;	/* process to interrupt later */
  delta_ticks = m_ptr->DELTA_TICKS;	/* 设定报警等待时间.how many ticks to wait */
  function = m_ptr->FUNC_TO_CALL;	/* 指定报警时的被调函数.function to call (tasks only) */
  rp = proc_addr(proc_nr);
  mc.SECONDS_LEFT = (rp->p_alarm == 0L ? 0 : (rp->p_alarm - realtime)/HZ );
  rp->p_alarm = (delta_ticks == 0L ? 0L : realtime + delta_ticks);
  if (proc_nr < 0) watch_dog[-proc_nr] = function;//将被调函数指针存入watch_dog数组.

  /* Which alarm is next? */
  next_alarm = MAX_P_LONG;//先设它为最大long值.
  for (rp = &proc[0]; rp < &proc[NR_TASKS+NR_PROCS]; rp++)//遍历所有进程,
...
{
/* This routine called on every clock tick(时钟中断). */
