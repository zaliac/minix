/* 9 General constants used by the kernel. */
...
/* 中断向量 Interrupt vectors */
#define DIVIDE_VECTOR      0	/* 除法 divide interrupt vector */
#define CLOCK_VECTOR       8	/* 时钟 clock interrupt vector */
#define KEYBOARD_VECTOR    9	/* 键盘 keyboard interrupt vector */
#define XT_WINI_VECTOR	  13	/* xt winchester interrupt vector */
#define FLOPPY_VECTOR     14	/* 软驱 floppy disk interrupt vector */
#define PRINTER_VECTOR    15	/* 打印机 line printer interrupt vector */
#define SYS_VECTOR        32	/* 系统调用 system calls are made with int SYSVEC */
#define AT_WINI_VECTOR	 118	/* at winchester interrupt vector */

/* 8259中断控制器需重新设定.The 8259A interrupt controller has to be re-enabled after each interrupt. */
...
#define TASK_STACK_BYTES 256	/* 任务堆栈大小 how many bytes for each task stack */
#define K_STACK_BYTES    256	/* 内核堆栈大小 how many bytes for the kernel stack */

#define RET_REG            0	/* system call return codes go in this reg */
#define IDLE            -999	/* 'cur_proc' = IDLE means nobody is running */

/* The following items pertain相关 to the 3 scheduling queues. */
#define NQ                 3	/* 3个优先队列. # of scheduling queues */
#define TASK_Q             0	/* 最高优先级 ready tasks are scheduled via queue 0 */
#define SERVER_Q           1	/* 中等优先级 ready servers are scheduled via queue 1 */
#define USER_Q             2	/* 最低优先级 ready users are scheduled via queue 2 */
