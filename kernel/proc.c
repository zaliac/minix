/* 11.2 进程调度?This file contains essentially all of the process and message handling.
 * It has two main entry points from the outside:
 *
 *   sys_call系统调用:   called when a process or task does SEND, RECEIVE or SENDREC
 *   interrupt中断:	called by interrupt routines to send a message to task
 *
 * It also has five minor entry points:
 *
 *   ready就绪:	put a process on one of the ready queues so it can be run
 *   unready冻结:	remove a process from the ready queues
 *   sched调度:	a process has run too long; schedule another one
 *   mini_send:	send a message (used by interrupt signals, etc.)中断信号发送消息.
 *   pick_proc:	pick a process to run (used by system initialization)挑选一个进程运行.
 */
...
{
/* An interrupt has occurred.  Schedule the task that handles it.中断产生,查找相关任务处理它 */

  int i, n, old_map, this_bit;

#ifdef i8088
  /* 1 重新启动中断控制器8259.Re-enable the 8259A interrupt controller. */
  ...
  /* 2 调用mini_send发送消息.Try to send the interrupt message to the indicated task. */
  this_bit = 1 << (-task);
  ...
  /* 决定下一步哪个任务或进程执行.If a task has just been readied and a user is running, run the task. */
  if (rdy_head[TASK_Q] != NIL_PROC && (cur_proc >= 0 || cur_proc == IDLE))
	pick_proc();//调用mpx88.s中的_restore,装配下一个要执行的任务或进程的各个寄存器,并启动它.
}
...
message *m_ptr;			/* pointer to message 在内存的缓存区*/
{
 ...
 */
  ...
  if ( (dest_ptr->p_flags & RECEIVING) &&
	...
	if (dest_ptr->p_flags == 0) ready(dest_ptr);	/*将目标任务设为就绪状态.*/
  } else {
