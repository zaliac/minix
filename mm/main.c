/* 6 存储模块主程序.This file contains the main program of the memory manager and some related
...
/* Main routine of the memory manager. */
  ...
  mm_init();			/* 1 初使化.initialize memory manager tables */

  /* 2 进入循环.This is MM's main loop-  get work and do it, forever and forever. */
  while (TRUE) {
	/* Wait for message. */
