/* 3 进程表(存储管理模块)结构定义.This table has one slot per process.  It contains all the memory management
...
EXTERN struct mproc {
  struct mem_map mp_seg[NR_SEGS];	/* 指向正文段,数据段,栈.points to text, data, stack */
  ...
  /* 位图,含有16位,每位表示一个信号. Bit maps for signals. */
