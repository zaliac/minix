/* 7 fork()实现.This file deals with creating processes (via FORK) and deleting them (via
...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  */
  ...
  if ( (child_base = alloc_mem(prog_clicks)) == NO_MEM) return(EAGAIN);//为子进程分配内存.
  ...
  rmc->mp_seg[T].mem_phys = child_base;//更新子进程内存映像地址.原地址是父进程的.
